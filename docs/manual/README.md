...
在当前工作台主页新建项目，然后打开「文件 → 导入 JSON」，选择以下文件。建议先另存自己的名称。导入会创建独立项目，原项目保留；未保存草稿不属于导出文件。
...
| [09 五重凹切](examples/09-multi-index-concave.json) | 保留五重方向，组合平切与可停用凹切 | 120 齿、16 个有效平面；半径 0.28 的五重球形凹切，另含停用圆柱备选 |
...
封面、实时预览、光学观察、文件交付与试作展示使用 **PC 01.338 Eight Main Highlight**，设计者 **Long, R H & Steele, N W**，来源记录为 *Facet Design v5 (1984) pC12*。在「首页 → 新建项目 → 从预设琢型开始」搜索名称并载入；[对应可编辑 JSON](../../public/presets/documents/94504-pc-01-338-eight-main-highlight.json) 与 [来源记录](../../public/presets/catalog.json) 随仓库提供。截图取自真实工作台，未改动几何；简化练习仅用于讲解切割过程与 Meet 操作。
...
1. 从主页“新建项目 → 预设底胚 → 心形 → 新建项目”开始。保持默认缩放，记住眼前的凹口或一条棱。分别向右、左、下、上短距离拖动，判断眼前部分是否跟随输入方向转动。方向键应有相同方向。
2. 切换“光学仿真”重复操作。参照实体轮廓和棱，避免把反射亮斑当作固定的刻面标记；相同材料、环境和缩放下比较前后画面。
3. 从台面、亭部、正视、侧视继续拖动。编辑与光学都在上下观察边界停止，反向拖动应平滑离开。精确轴向的水平拖动会绕观察轴转动，不是横向平移。
4. Shift + 拖拽应平移，滚轮应缩放，0 应复位。再新建冠部切割，将深度设为 0.100；旋转和切换光学返回后，参数及未保存状态应保持不变。拖动粉色深度手柄应调整切割，而不是转动观察视角；取消草稿可恢复起点。
