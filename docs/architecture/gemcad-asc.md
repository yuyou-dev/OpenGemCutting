...
导入 OBJ 初始化的 mesh 项目一律阻断 ASC 导出，即使当前外形看起来已凸，也不绕过预检。ASC 不能完整保存初始凹原石、面片和 CUT 历史；请使用 JSON v2 继续设计或交接。普通 cube 文档的 ASC 导入／导出规则保持不变。
...
- 缺少 `0°` 台面或包含多个 `0°` 台面的 ASC 当前阻断导入；负距离的 `0°` 底尖属于亭部，不计入台面数量。静默补台面或猜测固定 T1 都会虚构源工序；支持无台面、多台面设计需要未来显式扩展文档不变量。
- preform 记录当前阻断。应先在 GemCad 中把预形复制为真实刻面。

## 导出规则

只导出完整已提交实体中仍有效的平面；被覆盖的历史面不导出，临时显隐不改变出口几何。已锁 Meet 的来源、比例点和预形用途不随 ASC 传递，预检须明确省略范围。

导出固定写出 `GemCad 5.0`、`g96 0.0`、显式 tier 索引、折射率、标题、脚注和切割说明。每组有效刻面必须共享行业角与平面距离；否则不能表示为单个 ASC tier 并阻断导出。

导入和导出前都显示持久预检：源齿轮／96 齿精确映射、统一比例、L/W、层／有效面数、折射率与诊断。错误阻断，警告允许用户明确选择继续。
