...
- 应用页面为主页、切型编辑、实验室、格式中心，归 `src/App.jsx` 的页面 chrome；启动进入主页。页面导航不属于 CUT 会话，也不改变几何、提交/取消草稿或写入撤销历史。
- 当前编辑器保持挂载；去主页、实验室或格式中心只暂停绘制、拾取与键盘事件，返回同一项目恢复原 CUT、Meet、相机与历史。切换到另一项目或新建项目才建立独立编辑器；若当前存在未保存预览，先明确保留当前现场或放弃预览后切换。
...
- 格式中心（`FormatCenterPage`，规则见 [格式中心](format-center.md)）只读取 App 的已提交快照 `latestDocument` 或用户选择的文件；来源、去向与报告是页面局部状态，离开即丢弃，不写入项目、历史或 CUT 会话。未提交预览不参与导出并在来源处说明。“在工作台打开”只把新文档交给 App 的 `importCrystal`，经既有未保存预览确认后新建独立项目，原项目不变；导出只下载文件。进入意图（当前项目／选择文件）由文件菜单传入，不是持久状态。
