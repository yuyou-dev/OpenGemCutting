...
`OpticsViewport` 优先异步加载 vgpu / WebGPU，同时保留 WebGL2。后端选择只属于视口会话，不写入 document、CUT 状态或历史。初始化期间合并到最新的实体、材质和相机快照；没有 WebGPU、初始化/绘制失败或 device lost 时，由 React 创建新的 canvas 并切回 WebGL2。相机、观察位、材质和当前实体保留，pointer、wheel、touch 监听重新绑定；WebGL2 继续使用原有 context lost/restored 恢复逻辑。卸载后到达的异步初始化结果必须释放，不得触发回退或重绘。

两个后端共用领域几何和 BVH 打包，分别保留凸半空间求交与真实网格遍历，不以凸包替代凹体或遗漏分离组件。分辨率、单像素一次采样、反射次数、材质单位和灯光保持一致。一次最多提交一个待完成帧，输入更新合并；静止时不轮询或重复绘制。WebGPU 的缓冲缓存只在完整上传及绘制验证完成后接受对应的不可变 geometry。存储容量超限时回退，不截断几何。
