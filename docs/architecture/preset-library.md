...
预设库从“首页 → 新建项目 → 从预设琢型开始”打开居中浏览层。设计师先按轮廓、有效面数和真实长宽比缩小范围，再比较真实几何四视图、基础参数和作者来源，最后选择“以此琢型新建项目”作为研究起点；原项目完整保留。

当前内置 catalog 为 252 项、27 类外形和 1,008 张四视图，数据以 `public/presets/catalog.json` 为准。文档职责见 [索引](../README.md)。

## 产品决策

- 收录归档内全部通过当前质量门槛且几何唯一的 ASC；正式构建至少 50 项，无固定收录上限，数量不能替代质量。历史扩充过程保留在版本记录中。
...
起点：保留当前保存文档；打开“首页 → 新建项目 → 从预设琢型开始”。目标是找到细长椭圆的研究起点，同时把有效刻面限制在100面以内，便于比较刻面组织。
