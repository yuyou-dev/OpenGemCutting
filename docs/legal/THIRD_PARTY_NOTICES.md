...
- vgpu and its @vgpu runtime packages are MIT licensed; see [vercel-labs/vgpu](https://github.com/vercel-labs/vgpu) and the licenses shipped in the installed packages. The optical WebGPU backend is loaded only when its viewport is opened.
