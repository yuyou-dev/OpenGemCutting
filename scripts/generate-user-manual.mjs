...
const pageChrome = (section, pageNumber) => `
  <header class="page-header"><span>${section}</span><span>SUVA · FACET 96</span></header>
...
const cover = `
  <section class="page cover">
    ...
    <div class="cover-meta"><span>适用版本 <b>v${packageJson.version}</b></span><span>读者 琢型设计师与宝石切磨学习者</span><span>2026-09</span></div>
  </section>`;
...
const concepts = standardPage({
  ...
  ])}${figure(screenshots.workspaceTop, "图 3  完成切型的顶视轮廓，可直接核对对称性", [], "portrait-crop")}</div>${callout("为什么要理解图层", "撤销、重排、显隐、ASC 交换和 PDF 工艺表都从 CUT STACK 派生。只看最终网格，会丢失继续编辑所需的设计语义。")}`,
});
...
const checklist = standardPage({
  ...
  ].map(([n, title, text]) => `<div><span>${n}</span><i></i><p><strong>${title}</strong>${text}</p></div>`).join("")}</div><div class="archive-box"><div><small>RECOMMENDED ARCHIVE</small><h3>推荐归档组合</h3></div><ol><li>JSON 完整主文件</li><li>PDF 技术报告</li><li>必要时附 GemCad ASC</li></ol></div>${callout("最终原则", "ASC 不替代 JSON，截图不替代文档，漂亮的光学画面也不替代几何检查。")}`,
});
