...
const screenshots = {
  ...
  recovery: shot("09-local-recovery.jpg"),
};
...
const pageChrome = (section, pageNumber) => `
  ...
  <footer class="page-footer"><span>切磨工作台 · 操作手册 · v${packageJson.version}</span><span><b>${String(pageNumber).padStart(2, "0")}</b> / 13</span></footer>`;
...
const contents = standardPage({
  ...
  body: `<div class="toc">${[
    ...
    ["5.1", "本地恢复", "自动备份 · 多标签页 · 恢复边界", "09"],
    ["6", "聚焦光学仿真", "材质 · 观察视角 · 判断边界", "10"],
    ["6.1", "台面观察与对照", "完成切型的真实光学图例", "11"],
    ["7", "帮助与快捷操作", "任务式帮助 · 键盘与视口", "12"],
    ["8", "交付前检查", "几何 · 文件 · 报告 · 可恢复性", "13"],
...
const orientation = standardPage({
  ...
  body: `<p class="lead">切磨工作台不是自由建模器。你编辑的是一份有顺序、可撤销、可交换的参数化切割文档；画布中的宝石只是这份文档的实时结果。</p>${numberRows([
    ...
    ["02", "一次只处理一个 CUT 会话", "选择区域后新建；点击图层参数或面数编辑，点击名称改名。参数修改前保留已提交几何。"],
    ["03", "先看实体，再保存", "核对有效 / 生成面数、体积与切割指令；命令条只统计已提交刻面，视口另列残余毛坯面。"],
    ["04", "用正确格式交付", "JSON 是完整主文件；ASC 用于 GemCad 交换；PDF 用于阅读、打印和工艺沟通。"],
  ])}<div class="journey"><span><b>01</b> 选择起点</span><i></i><span><b>02</b> 创建 CUT</span><i></i><span><b>03</b> 几何检查</span><i></i><span><b>04</b> 光学观察</span><i></i><span><b>05</b> 保存交付</span></div>${callout("安全原则", "预设载入和文档级变换都会作为一次命令进入历史；Escape 优先关闭刻面表等浮层，再用于取消 CUT；浮层关闭不丢弃草稿。", "green")}`,
});
...
const concepts = standardPage({
  ...
  body: `<div class="concept-layout">${termRows([
    ...
    ["重复", "REPEAT", "让同一图层围绕轴心产生 N 个裁切平面；最终有效面取决于后续工序。"],
...
const cutWorkflow = standardPage({
  ...
  body: `${figure(screenshots.workspace, "图 4  在已完成的水滴形上新建冠部 CUT：Gizmo、主切割面与会话槽位同时出现", [
    ...
    ["02", "预告、定位或锁定", "新建冠部/亭部从 0.000 深度开始；空心菱形预告下一点，执行 Jump 后切面才移动，也可锁定单顶点 Meet。"],
  ])}${numberRows([
    ["03", "检查预览", "所有重复面参与几何，但主 Gizmo 始终对应基础索引。"],
    ["04", "保存或取消", "只有参数确实变化后才能提交；Escape 取消且不写历史。"],
  ])}</div>${callout("Meet 深度由系统求解", "锁定顶点后深度只读；角度、基础索引、重复或镜像变化会自动补偿。不可达、来源失效、空切或结构层失效时不能保存；普通冠部/亭部覆盖按影响程度提示或确认。", "green")}`,
});
...
const files = standardPage({
  ...
  ], "medium")}<div class="format-cards"><article><span>01</span><h3>JSON 完整主文件</h3><p>保留文档名称、毛坯、CUT STACK、显式刻面与光学元数据。适合归档与继续编辑；所有导出都不包含未保存预览。</p><b>默认选择</b></article><article><span>02</span><h3>GemCad ASC</h3><p>交换角度、索引、距离比例与折射率。重复和镜像会展开为显式切面。</p><b>行业交换</b></article><article><span>03</span><h3>PDF 技术报告</h3><p>输出工程视图、尺寸和逐面表。适合打印、复核和工艺沟通。</p><b>阅读交付</b></article></div>${callout("本地恢复与文件保存", "提交后的文档与材质自动备份；启动提示恢复，各页面独立保存，文件菜单可查看。草稿、相机、旧历史不恢复；备份只属于当前浏览器与站点，长期归档请导出 JSON。")}`,
});

const recovery = standardPage({
  section: "§5.1 · 本地恢复", index: "5.1", title: "找回已提交的设计", subtitle: "LOCAL RECOVERY · EACH PAGE SAVES ITS OWN COPY", pageNumber: 9,
  body: `<p class="lead">提交切割、修改材质或载入文档后自动备份。重新打开工作台会提示恢复；你也可以从文件菜单打开本地备份列表。</p>${figure(screenshots.recovery, "本地恢复：按更新时间选择已提交的设计", [], "medium")}${numberRows([
    ["01", "确认要恢复的版本", "根据名称、更新时间和层数选择备份。恢复会在当前页面建立独立副本，其他标签页的记录不被覆盖。"],
    ["02", "恢复文档与材质", "已提交 CUT STACK、Meet 快照和光学材质一起恢复。此次载入可以一步撤销；此前的旧撤销历史不恢复。"],
    ["03", "重新开始未完成操作", "未保存 CUT 草稿、群组预览、相机和 VIEW ONLY 参数不属于备份。继续设计前重新确认工作状态。"],
    ["04", "保留长期主文件", "本地备份属于当前浏览器与站点地址。空间不足或无法访问时按提示重试或导出 JSON；重要设计始终单独归档。"],
  ])}${callout("删除之前先归档", "恢复列表只会在明确确认后删除选中的记录。删除备份不改变当前工作台文档，但该备份不能再用于恢复。", "green")}`,
});

const opticsMaterial = standardPage({
  section: "§6 · 光学仿真", index: "§6", title: "聚焦光学仿真", subtitle: "REAL GEOMETRY · MATERIAL · LIGHT", pageNumber: 10,
  ...
  ], "hero")}<div class="legend-grid compact"><div><span>1</span><p><strong>真实几何</strong>当前 77 面预设切型直接参与光学计算</p></div><div><span>2</span><p><strong>材质常量</strong>折射率、色散与临界角保持联动</p></div><div><span>3</span><p><strong>体色与吸收</strong>使用 Beer-Lambert 模型观察光程影响</p></div><div><span>4</span><p><strong>观察命令条</strong>视角、切型名称、退出与实时状态</p></div></div>${callout("判断边界", "曝光和摄影棚只改变观看条件，不会修复破面、漏光或错误的 CUT 顺序。材质随文档保存与撤销；先保证几何正确，再比较材料表现。", "green")}`,
});

const opticsViews = standardPage({
  section: "§6.1 · 光学观察", index: "6.1", title: "用视角完成判断", subtitle: "TABLE · PAVILION · FRONT · SIDE", pageNumber: 11,
...
const help = standardPage({
  section: "§7 · 帮助与快捷键", index: "§7", title: "随时回到正确下一步", subtitle: "TASK-BASED HELP · STAY IN CONTEXT", pageNumber: 12,
  body: `${figure(screenshots.help, "图 8  更多 → 帮助与操作手册：按任务组织，不离开当前编辑现场", [
    ...
    ["0", "复位相机"], ["J / Shift+J", "下一 / 上一交点"], ["Escape", "退出拾取、取消 CUT 或退出光学仿真"],
...
const checklist = standardPage({
  section: "§8 · 交付检查", index: "§8", title: "完成设计前的检查清单", subtitle: "GEOMETRY · DOCUMENT · EXCHANGE · REPORT", pageNumber: 13,
...
</style></head><body>${cover}${contents}${orientation}${map}${quickStart}${concepts}${cutWorkflow}${files}${recovery}${opticsMaterial}${opticsViews}${help}${checklist}</body></html>`;
...
console.log(`Generated ${path.relative(root, outputPath)} (13 pages)`);
