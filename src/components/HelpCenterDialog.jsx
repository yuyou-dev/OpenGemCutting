...
const HELP_SECTIONS = [
  {
    ...
    steps: [
      ["01", "打开或创建项目", "首次打开会准备两份可编辑的琢型项目。点击卡片继续，或用左侧“＋新建项目”选择默认起点、预设琢型、预设底胚或上传 OBJ。新建卡后从左向右显示最近三个项目，换行从最左列继续，“全部”展开其余作品；搜索会显示全部匹配项。四种预设高度为最大横向尺寸的 100%，A 形保留贯穿孔；预设从零切割开始。卡片左上角的删除入口会经确认后从此浏览器移除项目，不可撤销，重要设计请先导出 JSON。已有本地备份会迁入项目列表，原备份仍保留。"],
      ["02", "找到接近想法的切型", "首页 → 新建项目 → 从预设琢型开始，用外形、有效面数和 L/W 长宽比缩小范围，也可组合名称、作者或来源关键词。逐页比较四视图，点击“以此琢型新建项目”并命名；原项目保留；先导出 JSON 留住基准版。"],
      ["03", "改变一个设计要素", "右侧 CUT STACK 列出设计步骤，内容多时可滚动。点击图层旁的“编辑”，在左侧调角度或深度；想增加一组刻面，则用区域标签下方绿色的“新增切割图层”按钮。"],
    ...
    note: "项目主页、编辑工作台与实验室沿用同一界面规范。回主页或去实验室会保留本页编辑现场，不会替你保存 CUT；切换到另一项目或刷新时，未保存预览不作为项目内容恢复。",
  },
  {
    ...
    facts: [
      ...
      ["重复与镜像", "先在新建项目中选择设备分度盘。全部平面切割使用同一盘，腰棱也遵循它。用重复建立节奏，用镜像增加成对关系；主切面是操纵杆与 Meet 的参考。"],
      ["检查分度兼容", "左侧齿数旁绿灯表示符合整齿，红灯表示不符合。换盘时保留原造型并统一换算所有平切读数；旧方向无法落在整齿时仍保留，只显示红灯。"],
      ["制作五重凹切", "右侧切到凹切，点击添加凹切选择球形、圆弧槽、V 形尖槽、细槽或三角柱尖槽。每组成为独立图层；拖动角度转盘和深度实时预览，松手保存。角度不受平切分度盘限制；三角柱可调尖角、宽度和长度，尺寸变化保持当前切深。启停和删除均可撤销。"],
      ["固定底胚，独立设计", "底胚只在新建项目时选择，创建后不能更换或缩放；没有选择则只设计切型。平切与凹切使用固定坐标，各自保存；凹切不影响平切 Meet／Jump 约束。平切面板的凹切显隐只改变观察，不改设计与导出。所有导出都在顶部文件菜单。"],
    ...
    note: "先看新增刻面是否实际形成、是否覆盖了你想保留的形状，再点击“加入序列／保存”。新建仅接触且有效 0 面不能保存；覆盖任何旧层均可保存，原工序留在列表中并可撤销。已被覆盖的旧层仍可编辑。",
  },
  {
    ...
    facts: [
      ...
      ["对称切割凹形原石", "重复与镜像安排切割方向，不会复制或补齐原石。切后可能形成多片切面或分离碎块，全部保留；清除切割回到导入时的原石。用 JSON 保存原石与完整工序，导入晶体项目不能导出 ASC 或 GCS，格式中心会明确说明。"],
      ["继续设计用 JSON", "保留完整平切、可选底胚和凹切，以及所选分度盘与兼容盘。读数统一按所选盘输出；未声明按 96 解读，非 96 或不兼容 96 会提醒。导入完整 JSON 会新建项目，原项目保留。"],
      ...
      ["保留旧设计", "升级前的本地备份会一次性迁入项目，原备份仍可从“文件 → 恢复本地设计”读取，当前版本不再新增旧版备份。手动恢复会创建独立项目，原项目和来源备份保留。删除迁入项目后，同一备份不会再次迁入。"],
      ["返回或切换项目", "返回主页后“继续当前设计”可保留本页草稿与撤销历史；切换另一项目或新建时，会先确认是否放弃未保存预览，并为打开的项目重新建立撤销历史。"],
      ["讨论工艺用 PDF", "导出当前切型的工艺报告，查看真实视图、有效刻面参数及 Meet 来源。报告含实际分度与独立凹切工具参数附录，用于阅读与沟通，不能代替 JSON 继续编辑。设计含磨砂面时，默认按全抛光导出，也可选择标注磨砂面。"],
      ["与 GemCAD、Gem Cut Studio 交换", "文件 →“从 GemCAD / Gem Cut Studio 导入…”或“导出到其他软件…”进入格式中心。先选来源（当前项目，或 .asc、.gem、.gcs 文件），再看每个去向会保留、简化或丢失什么：ASC 没有磨砂面，GCS 只有磨砂开关，两者都不保存 Meet 构造。GemCAD .gem 只能读取；没有唯一台面的设计可以直接互转，但不能在工作台打开。"],
      ["从练习到自己的作品", "手册中的案例 JSON 可作为练习起点；理解每次变化后，再在自己的切型上比较。练习参数不代表所有宝石都适用。"],
    ],
    note: "JSON、ASC、GCS、PDF 都只导出已保存的设计，预览不会自动提交。本地项目只属于当前浏览器和站点地址，不能代替长期归档。保存失败时可重试、导出 JSON 或回主页删除不需要的项目释放空间，不能把未成功写入的内容当作已保存。",
  },
  {
    ...
    steps: [
      ...
      ["03", "比较磨砂与抛光", "进入时默认“全抛光”。在画布左上切到“按设计表面”，查看实验室已保存的磨砂标注；未标注的面仍按抛光显示。保持相同材质、环境和视角，停下等待画面清晰后比较。切换只影响预览，不修改标注、切割参数或撤销记录。"],
      ["04", "比较并记录取舍", "在画布右上水平条切换透视、台面、亭部、正视和侧视，再转动宝石。记录哪种变化更符合设计目标，不只凭某一帧的亮度决定。"],
      ["05", "返回继续设计", "退出仿真或按 Escape，回到原编辑现场。需要留住当前造型时，回到工作台再保存。"],
    ],
    note: "从主页或编辑页进入实验室，先保存或放弃未保存切割，再带入当前设计或新建实验。实验稿独立自动保存；返回候选后由工作台检查，再带回为新项目。来源保持原样，最近实验可继续或下载恢复文件。现有光学仿真仍从画布左上模式切换组进入。",
  },
  {
    ...
    facts: [
      ...
      ["返回项目与反馈", "浮动菜单可点击外部或按 Esc 关闭。顶部可返回项目主页或进入实验室。“GitHub 仓库”始终保留，三个页面和光学仿真均可使用；点击在新标签页打开 OpenGemCutting，当前设计留在原页。"],
      ...
      ["逐步观看切割过程", "画布左上模式切换组 → 切割助手。用画布底部播放条的“下一步 / 上一步”逐刀观看已提交设计的切割，或用“下一组 / 上一组”按图层跳转；点击或拖动进度条可到任意进度；中央播放键自动逐刀演示，支持 0.5× / 1× / 2×，手动步进和切到后台会暂停。右侧仪表显示下一刀的分度、行业角、深度与本组刀序。磨砂面显示“磨砂切割 · 无需抛光”，只提醒表面处理，不增加刀数。切完保留原视角 0.5 秒，再转向下一刀；停稳后才显示粉色平面。跟随当前面以侧偏 45° 斜向观察，可调转场速度；拖动画布会暂停跟随，可用开关恢复。步数表示已完成刀数，粉色平面提示下一刀。助手只读，不会改动设计或当前 CUT。"],
