...
const HELP_SECTIONS = [
  ...
  },
  {
    ...
    steps: [
      ["01", "进入观察", "画布左上模式切换组 → 光学仿真（显示模式菜单保留同一入口）。当前保存或预览形状会进入仿真，切割工具暂时收起。"],
      ["02", "选定材质与环境", "选择材质预设，确认体色、折射率等设置；先固定环境和观察角度，再比较不同切型。"],
      ["03", "比较并记录取舍", "在画布右缘切换台面、亭部、正视和透视，再转动宝石。记录哪种变化更符合设计目标，不只凭某一帧的亮度决定。"],
  ...
  },
  {
    ...
    facts: [
      ...
      ["逐步观看切割过程", "画布左上模式切换组 → 切割助手 β。用画布底部播放条的“下一步 / 上一步”逐刀观看已提交设计的切割，或用“下一组 / 上一组”按图层跳转；点击播放条的进度条可到任意进度。助手只读，不会改动设计或当前 CUT。"],
