...
const HELP_SECTIONS = [
  ...
  },
  {
    ...
    facts: [
      ...
      ["逐步观看切割过程", "画布左上模式切换组 → 切割助手。用画布底部播放条的“下一步 / 上一步”逐刀观看已提交设计的切割，或用“下一组 / 上一组”按图层跳转；点击或拖动进度条可到任意进度；中央播放键自动逐刀演示，支持 0.5× / 1× / 2×，手动步进和切到后台会暂停。右侧仪表显示下一刀的分度、行业角、深度与本组刀序。跟随当前面以 45° 斜向观察，可调转场速度；拖动画布会暂停跟随，可用开关恢复。步数表示已完成刀数，粉色平面提示下一刀。助手只读，不会改动设计或当前 CUT。"],
