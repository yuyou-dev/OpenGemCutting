...
const HELP_SECTIONS = [
  {
    ...
    note: "打开菜单、帮助和预设库不会改动文档；载入预设会作为一次可撤销命令记录。",
  },
  {
    id: "cut",
    label: "切割工作流",
    ...
    facts: [
      ...
      ["编辑图层", "先读取已保存参数，真正修改后才出现未保存预览。"],
    ],
    note: "任何活动会话都可用 Escape 取消；取消不会写入历史。",
  },
  {
    id: "files",
    label: "文件与交换",
    ...
    facts: [
      ["JSON", "完整主文件，保留文档、参数化图层与光学元数据。"],
      ["GemCad ASC", "用于交换显式 96 齿平面；导入导出前必须通过持久预检。"],
      ["PDF 技术报告", "输出五视图、尺寸与按图层组织的逐面参数，适合工艺交付。"],
      ["预设琢型", "来自公开来源、已经过格式与几何检查，可一键载入并撤销。"],
    ],
    note: "ASC 不保存历史、隐藏状态、毛坯、会话与完整光学元数据；需要继续编辑时优先保留 JSON。",
  },
  {
    id: "optics",
    label: "光学仿真",
    ...
    note: "仿真设置不会改变切割几何；VIEW ONLY 参数只影响观察环境。",
  },
  {
    id: "keys",
    label: "视口与快捷键",
