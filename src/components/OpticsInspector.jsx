...
function RangeField({ label, code, value, min, max, step, unit, digits = 3, onChange }) {
  return (
    <label className="optics-range-field">
      <span className="optics-field-label"><strong>{t(label)}</strong><small>{t(code)}</small></span>
      <input
        type="range"
        min={min}
        max={max}
...
export function OpticsInspector({ settings, tab, onTabChange, onChange, onClose }) {
  ...
  return (
    <aside className="optics-inspector" aria-label={t("光学仿真检查器")}>
      <div className="optics-inspector__heading">
        <span><IconDiamond size={17} stroke={1.6} /><strong>{t("光学仿真")}</strong><small>OPTICS</small></span>
        <button type="button" onClick={onClose} aria-label={t("收起光学参数")}><IconX size={16} stroke={1.7} /></button>
      </div>
      <div className="optics-tabs" role="tablist" aria-label={t("光学参数分类")}>
        <button type="button" role="tab" aria-selected={tab === "material"} className={tab === "material" ? "is-active" : ""} onClick={() => onTabChange("material")}> {t("材质 MATERIAL")} </button>
        <button type="button" role="tab" aria-selected={tab === "view"} className={tab === "view" ? "is-active" : ""} onClick={() => onTabChange("view")}> {t("视图 VIEW")} </button>
      </div>

      {tab === "material" ? (
        <div className="optics-inspector__body" role="tabpanel">
          <label className="optics-select-field">
            <span><strong>{t("材质预设")}</strong><small>PRESET</small></span>
            <select
          ...
          <div className="optics-physical-card">
            <div className="optics-card-title"><IconSparkles size={14} stroke={1.7} /><span>{t("物理材质常量")}</span><small>PHYSICAL</small></div>
            <RangeField label={t("折射率")} code="IOR · nD" value={resolved.material.ior} min="1.001" max="3.5" step="0.001" onChange={(ior) => updateMaterial({ ior, preset: "custom" })} />
            <RangeField label={t("色散")} code="DISPERSION · B−G" value={resolved.material.dispersion} min="0" max="0.15" step="0.001" onChange={(dispersion) => updateMaterial({ dispersion, preset: "custom" })} />
            <div className="optics-derived-readout">
              <span>{t("临界角")} <small>CRITICAL ANGLE</small></span>
              <strong>{criticalAngleDegrees(resolved.material.ior).toFixed(1)}°</strong>
            </div>
          </div>

          <fieldset className="optics-color-field">
            <legend><strong>{t("体色")}</strong><small>BODY COLOR / ABSORPTION TINT</small></legend>
            ...
            <details className="optics-color-picker">
              <summary>
                ...
                <span>{t("自定义体色")} <small>COLOR PICKER</small></span>
          ...
          <RangeField label={t("吸收系数")} code="BEER–LAMBERT" value={resolved.material.absorption} min="0" max="0.5" step="0.005" unit="mm⁻¹" onChange={(absorption) => updateMaterial({ absorption, preset: "custom" })} />

          <details className="optics-advanced">
            <summary><IconAdjustments size={14} stroke={1.6} /><span>{t("高级计算")}</span><small>ADVANCED</small></summary>
            <RangeField label={t("内部反射")} code="MAX BOUNCES" value={resolved.advanced.maxBounces} min="2" max="8" step="1" digits={0} onChange={(maxBounces) => updateAdvanced({ maxBounces })} />
          </details>
        </div>
      ) : (
        <div className="optics-inspector__body" role="tabpanel">
          <div className="optics-view-section">
            <div className="optics-card-title"><IconSun size={14} stroke={1.7} /><span>{t("摄影棚环境")}</span><small>VIEW ONLY</small></div>
            <label className="optics-select-field is-compact">
              <span><strong>{t("环境")}</strong><small>ENVIRONMENT</small></span>
              <select value={resolved.view.environment} onChange={(event) => updateView({ environment: event.target.value })}>
                {OPTICAL_ENVIRONMENTS.map((environment) => <option key={environment.id} value={environment.id}>{t(environment.label)}</option>)}
              </select>
            </label>
            <label className="optics-select-field is-compact">
              <span><strong>{t("背景")}</strong><small>BACKGROUND</small></span>
              <select value={resolved.view.background} onChange={(event) => updateView({ background: event.target.value })}>
                {OPTICAL_BACKGROUNDS.map((background) => <option key={background.id} value={background.id}>{t(background.label)}</option>)}
              </select>
            </label>
            <RangeField label={t("曝光")} code="EXPOSURE" value={resolved.view.exposure} min="-2" max="2" step="0.05" unit="EV" digits={2} onChange={(exposure) => updateView({ exposure })} />
            <RangeField label={t("环境旋转")} code="ROTATION" value={resolved.view.environmentRotation} min="-180" max="180" step="1" unit="°" digits={0} onChange={(environmentRotation) => updateView({ environmentRotation })} />
          </div>
          <p className="optics-view-note">{t("环境、背景与曝光只影响观看，不改变当前文档的折射率、色散与吸收常量。")}</p>
        </div>
      )}

      <button type="button" className="optics-view-summary" onClick={() => onTabChange("view")}>
        <span>{t(environmentLabel)} · {t(backgroundLabel)}</span>
