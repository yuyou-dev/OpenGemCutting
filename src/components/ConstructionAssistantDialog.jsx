...
export function ConstructionAssistantDialog({
  ...
  const labelFor = (item) => item.label ?? item.operation?.label ?? t("施工工序");
  ...
  const sourceLabel = (target) => target.sourceOperationIds.map((id) => ["rough-cube", "rough-mesh"].includes(id) ? t("毛坯") : stages.find((item) => item.id === id)?.label ?? t("已删除工序")).join(" × ");
  return (
    <Modal title={t("逐层试切助理")} eyebrow="CONSTRUCTION REVIEW" className="construction-assistant-dialog" closeLabel={t("返回工作台")} onClose={onClose}>
      <p className="construction-assistant-scope">{t("按保存序列检查每道工序前后的真实实体。查看与切换阶段不会修改设计或当前 CUT。")}</p>
      {stage ? (
        <>
          <div className="construction-assistant-navigation">
            <button type="button" className="secondary-button" onClick={() => onStageChange(currentStageIndex - 1)} disabled={currentStageIndex === 0} aria-label={t("上一施工阶段")}><IconChevronLeft size={16} stroke={1.7} /></button>
            <label><span>{t("施工阶段")}</span><select aria-label={t("施工阶段")} value={currentStageIndex} onChange={(event) => onStageChange(Number(event.target.value))}>
              {stages.map((item, index) => <option key={item.id ?? item.operation?.id ?? index} value={index}>{index + 1} / {stages.length} · {labelFor(item)}{item.preform ? t(" · 预形") : ""}{item.construction && item.construction.status !== "valid" ? t(" · Meet 失效") : ""}</option>)}
            </select></label>
          ...
          <div className="construction-assistant-viewport" aria-label={t(`${labelFor(stage)} · ${phase === "before" ? t("切割前") : t("切割后")}`)}>{renderStage(stage, phase)}</div>
          ...
          {stage.hidden ? <p className="construction-assistant-hidden">{t("此工序当前已隐藏，切割前后实体保持一致。")}</p> : null}

          <p className={`construction-assistant-diagnostic${stale ? " is-stale" : ""}`} role="status">
            {t(stale ? stage.construction.message ?? t("Meet 来源已失效；已保存的明确切面保持不变。")
              : stage.construction ? t("Meet 来源与当前施工顺序一致。") : t("此工序未保存 Meet 约束。"))}
          </p>
          <p className="construction-assistant-footnote">{t("发现失效后，返回工作台编辑对应工序；助理不会自动修复或连带修改后续切面。")}</p>
          </div>
          </div>
        </>
      ) : <p className="construction-assistant-empty">{t("当前没有可查看的施工工序。")}</p>}
    </Modal>
  );
}
