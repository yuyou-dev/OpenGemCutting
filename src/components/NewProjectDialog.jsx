...
export function NewProjectDialog({ onClose, onDefault, onPreset, onUpload, onStartPreset, indexTeeth, onIndexTeethChange }) {
  ...
  const standard = useMemo(() => buildConstructionStages(createWorkbenchDocument('默认起点', indexTeeth)).at(-1).afterSolid, [indexTeeth]);
  ...
  return <div className="modal-backdrop" onMouseDown={e => { if (e.target === e.currentTarget) onClose(); }}>
    <section className="modal-panel stock-start-panel" ref={panelRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby={titleId}>
      ...
      <div className="stock-start-body">
        ...
        {mode === 'default' ? <div className="stock-start-default"><TechnicalPreview solid={standard} view="isometric" label={t("默认起点真实预览")}/><div><span className="stock-start-kicker">{t("熟悉的起点")}</span><h3>{t("从默认圆柱外观开始")}</h3><p>{t("沿用当前新建方式，包含默认台面与腰部。创建后即可安排下一道切割。")}</p><p className="stock-start-note">{t("此方式不使用实体底胚。底胚只能在新建时选择，创建后不可更换或缩放。")}</p></div></div> : <div className="stock-start-presets">
