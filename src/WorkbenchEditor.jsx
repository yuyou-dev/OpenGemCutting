...
export function WorkbenchEditor({ initialDocument, startWithDraft = false, visible = true, interactionPaused = false, onDocumentChange, onPreviewChange, onHome, onLab, onNewProject, projectStatus }) {
  ...
  const [sessionState, dispatchCutSession] = useReducer(
    cutSessionReducer,
    null,
    () => createCutSession(startWithDraft ? CUT_SESSION_MODE.CREATE : CUT_SESSION_MODE.IDLE, { region: "crown" }),
  );
    ...
    <main className={`app-shell workspace-editor${opticsActive ? " is-optics-active" : ""}`} hidden={!visible} aria-hidden={!visible} inert={!visible || interactionPaused}>
      ...
      <section className={`${sidebarOpen ? "editor-workspace" : "editor-workspace is-sidebar-collapsed"}${opticsActive ? " is-optics-focus" : ""}`}>
        ...
        {!opticsActive ? <aside className="workbench-right-sidebar" aria-label="切割序列与正交预览">
          ...
          />
          <OrthographicPreviews solid={displaySolid} activeOperationId={cutSession.activePatternId} previewOperationId={cutMode === "create" ? `draft-${patternMode}` : null} highlightOperationId={hoveredPatternId} />
        </aside> : null}
      </section>
