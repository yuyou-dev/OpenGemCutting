...
/** Version 1 per-facet finish: polished has zero roughness; frosted uses GGX alpha in [0.001, 1]. */
export function isSupportedSurfaceFinish(finish) {
  return object(finish) && finish.version === 1 && finish.model === 'ggx-dielectric'
...
export function validateDocumentExtensions(document) {
  ...
  for (const [entry, path] of entries) {
    ...
    if (finish !== undefined && !isSupportedSurfaceFinish(finish))
