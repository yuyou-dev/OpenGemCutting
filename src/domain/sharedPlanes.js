import { resolveFacetPattern, rotationalStockSupportOffset, normalizeIndex } from './faceting.js';
import { normalizeIndexTeeth } from './indexing.js';
...
export function indexCandidates(angleDegrees, radius = 2, indexTeeth = 96) {
  const step = 360 / normalizeIndexTeeth(indexTeeth);
  const center = Math.round(angleDegrees / step);
  return [...new Set(Array.from({length:2*radius+1},(_,i)=>normalizeIndex(center+i-radius, indexTeeth)))]
    .map(index=>({index,errorDegrees:Math.abs(((index*step-angleDegrees+180)%360+360)%360-180)}))
...
export function planeThroughNodes(index, a, b, region = 'crown', indexTeeth = 96) {
  const canonical = normalizeIndex(index, indexTeeth);
  const radians = canonical * Math.PI * 2 / indexTeeth;
...
export function patternFromPlane({normal,offset}, {id,label=id,region,baseIndex,indexTeeth=96,repeat=1,mirror=0,stock}) {
  ...
  const facets=resolveFacetPattern({patternId:id,label,region,baseIndex,indexTeeth,repeat,mirror,industryAngleDeg,depth},{stock});
  const actual=facets.find(f=>Math.abs(f.index-normalizeIndex(baseIndex,indexTeeth))<1e-9).plane;
