...
交付目标是设计师能继续修改的真实 CUT、原图等比例对照及逐项误差。先读仓库 AGENTS.md、docs/architecture/state-contract.md，检查脏工作区，保留已有设计。优先级是用户意图、可见节点/棱/面的连接和正交形状；角度、模板和面数只补充未知信息。非零体积、正腰厚、平面性和全局裁切可达性始终必需。
